...
munic_sf <- agriculture_munic %>%
  dplyr::filter(MUS_NM_MUN %in% munic_selection) %>%
  dplyr::select(municipalite = MUS_NM_MUN, geometry) %>%
...
data_agri_sf <- dplyr::left_join(munic_sf, agri_data, by = "municipalite") %>%
  st_transform(crs = target_crs)

# Clean duplicate municipalities by combining geometries
data_agri_sf <- data_agri_sf %>%
  dplyr::group_by(municipalite) %>%
  dplyr::summarise(
