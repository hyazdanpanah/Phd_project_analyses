...
Main light sources are identified using a **75th percentile threshold**, focusing on areas of significant development intensity (top 25% of light values within the coastal buffer). Sources are then **20 top sources** and weighted by their actual light intensity values.

```{r identify-light-sources}
# ===============================================================================
# SIMPLE APPROACH: THRESHOLD + TOP 20 BRIGHTEST POINTS
# ===============================================================================
...
# Convert ALL coastal sources to points 
...
cat("Total coastal light sources:", nrow(light_source_points), "\n")

# Apply 75th percentile threshold
threshold_75 <- quantile(light_source_points$light_intensity, 
                         probs = 0.75, na.rm = TRUE)

light_filtered <- light_source_points %>%
  filter(light_intensity >= threshold_75)

cat("After 75th percentile threshold:", nrow(light_filtered), "\n")
cat("Threshold value:", round(threshold_75, 3), "\n")

# Select TOP 20 BRIGHTEST points
light_top20 <- light_filtered %>%
  arrange(desc(light_intensity)) %>%
  slice(1:20)

cat("Selected top 20 brightest points\n")
cat("Intensity range:", 
    round(min(light_top20$light_intensity), 3), "-",
    round(max(light_top20$light_intensity), 3), "\n")

# Normalize intensity (0-1 scale)
max_light <- max(light_source_points$light_intensity, na.rm = TRUE)
light_top20$intensity_index <- light_top20$light_intensity / max_light

cat("Normalized intensity range:",
    round(min(light_top20$intensity_index), 3), "-",
    round(max(light_top20$intensity_index), 3), "\n")
```
...
# ===============================================================================
# CALCULATE EXPOSURE FOR TOP 20 POINTS WITH INTENSITY WEIGHTING
# ===============================================================================

decay_type <- "III"

cat("========================================\n")
cat("PROCESSING TOP 20 BRIGHTEST POINTS\n")
cat("========================================\n")

# Store individual exposures
point_exposures <- list()

# Calculate exposure for each of the 20 points
for (i in 1:nrow(light_top20)) {
  cat("Processing point", i, "/", nrow(light_top20),
      "- Intensity:", round(light_top20$intensity_index[i], 3), "\n")
  
  # Get intensity weight for this point
  intensity_weight <- light_top20$intensity_index[i]
  # Calculate relative exposure
  single_exposure <- RelativeExposure(
    source = light_top20[i,],
    transition = tr_water,
  ...
  # Weight by intensity
  point_exposures[[i]] <- single_exposure * intensity_weight
}

# Combine all exposures
cat("\n========================================\n")
cat("COMBINING ALL EXPOSURES\n")
cat("========================================\n")

combined_exposure <- Reduce('+', point_exposures)
  
# Normalize to 0-1 scale
max_val <- cellStats(combined_exposure, stat = "max", na.rm = TRUE)
combined_exposure <- combined_exposure / max_val

# Apply minimum value and mask
...
cat("\n=== SUMMARY ===\n")
cat("Total points processed:", nrow(light_top20), "\n")
cat("Intensity range:", 
    round(min(light_top20$intensity_index), 3), "-",
    round(max(light_top20$intensity_index), 3), "\n")
...
plot_exposure(coastal_exp,
              "Coastal Development Exposure -  20 top points (Level III)",
              xmin, xmax, ymin, ymax)
#dev.off()
```
# Cluster approach for entire SF 
...
## Setup Grid and Transition Layer
(already done in previous approach)
...
# Final exposure
coastal_exp <- combined_exposure
##chech the cluster positions : 
#plot(bathy_200, col = terrain.colors(100))
#plot(st_geometry(light_source_clustered), add = TRUE, pch = 19, col = "blue", cex = 2)
```
...
plot_exposure(coastal_exp,
              "Coastal Development Exposure -  inensity cluster approach (Level III)",
              xmin, xmax, ymin, ymax)
#dev.off()
```
